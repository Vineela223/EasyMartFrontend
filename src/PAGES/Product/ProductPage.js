...
const ProductPage = () => {
    ...
    const BASE_IMAGE_URL = 'http://localhost:8080'; // Backend base URL for images
    ...
    const getproductdatabyid = async () => {
        try {
            ...
            setproductdata(data);
            const resproductImages=data.productImages || [];
            setimageset(resproductImages);
            const allImages = resproductImages.flatMap((item) => [item.image1, item.image2, item.image3].filter(Boolean));

            setimageset(allImages);
            setactiveimg(allImages.length > 0 ? `${BASE_IMAGE_URL}${allImages[0]}` : "");
         } catch (error) {
    ...
    /*const allImages = productdata.productImages
        ? imageset.flatMap((item) => [item.image1, item.image2, item.image3].filter(Boolean))
        : [];*/
        ...
        <div className='productpage'>
            ...
            <div className='pc1'>
                ...
                <div className='c11'>
                    ...
                    {imageset.length > 0 && imageset.map((image, index) => ( // Only map if imageset has items
                        <div key={index} className='imgsmall' onClick={() => setactiveimg(`${BASE_IMAGE_URL}${image}`)}>
                            <img 
                                src={`${BASE_IMAGE_URL}${image}`} 
                                alt="Product" 
                                className={activeimg === `${BASE_IMAGE_URL}${image}` ? 'active' : ''} 
                            />
                        </div>
                    ))}
                    </div>
                    <div className='imgbig'>
                    {activeimg && <img src={activeimg} alt="Selected Product" />}
                </div>
                </div>
                <div className='c12'>
